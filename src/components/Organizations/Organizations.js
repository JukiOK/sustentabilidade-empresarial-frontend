        ...
        <tbody>
          {
            translateSector && translateCategory && orgList.map((org, index) => (
              <tr className="table-row" key={index} id={'table-row-' + index}
                onMouseEnter={() => isFilter && hoverEnter(index)}
                onMouseLeave={() => isFilter && hoverExit(index)}
                onClick={() => isFilter && setOrg(org)}
                >
